...
async function searchPhoneNumber(number) {
    number = number.trim();
    try {
        const client = await mongo.Phonebooks.find({ "_id": `${number}` });
        logger.access.info(`Со стороны базы вернулся результат ${util.inspect(result)}`);
        return client;
    } catch (e) {
    ...
    try {
        ...
        ari.on('StasisStart', (event) => {
            ...
            if (resultSearchClient == null) {
                ...
                await continueDialplan(event.channel.id, config.context.default, '00018');
                ...
                logger.access.info(`Был найден привязанный внутренний номер ${resultSearchClient['_id']}  ${resultSearchClient['company']}  ${resultSearchClient['fio']}  ${resultSearchClient['extension']}  вызов пошел по маршруту ${config.context.local}`);
                await continueDialplan(event.channel.id, config.context.local, resultSearchClient['extension']);
            }
        })
    } catch (e) {
