...
async function sendInfoToBitrix(localExtensionA, localExtensionB, startCall, billsec, isAnswered, recording) {
    try {
        ...
        logger.error.error(`Ошибка регистрации в Битрикс локального вызова  ${e}`);
    }
...
const server = net.createServer((connection) => {
    logger.access.info('Соединение');

    connection.on('data', (data) => {
        logger.access.info(`Получены CDR данные от АТС ${util.inspect(data.toString())}`);
        /* [ 'Call 52501',
            '2021/02/15 07:02:39',
            '00:00:15',
            '565',
        ...
        if (callCDR[3].length == 3 && localExtensionB[1].length == 3) {
            ...
            logger.access.info(Id3CXcall[1], startCall, duration, callCDR[3], localExtensionB[1])
            setTimeout(sendInfoByLocalCall, 18000, Id3CXcall[1], startCall, duration, callCDR[3], localExtensionB[1]);
        }
    });

    connection.on('close', () => {
        logger.access.info('client closed connection');
    });
}).listen(port);
...
server.on('error', (e) => {
    if (e.code == 'EADDRINUSE') {
        logger.error.error('Address in use, retrying...');
        setTimeout(() => {
            server.close();
            server.listen(port);
        }, 1000);
...
server.on('listening', function() {
    logger.access.info(`Сервер слушается на порту ${port}`);
});